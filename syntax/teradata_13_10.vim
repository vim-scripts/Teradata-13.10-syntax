" Vim syntax file
" Language:    Teradata SQL, BTEQ Commands (Teradata 13.10)
" Maintainer:  Milan Opath <milan.opath at gmail com>
...
syntax keyword SqlCmd2 DEL DELETE DEMOGRAPHICS DENIALS DESC DESCRIBE DETERMINISTIC DIAGNOSTIC
...
syntax keyword SqlCmd2 FALLBACK FASTEXPORT FETCH FINAL FIRST FOR FOREIGN FOUND FREESPACE FROM FULL
...
syntax keyword SqlCmd2 IDENTITY IF IFP IMMEDIATE IN INCONSISTENT INDEX INITIATE INNER INOUT INPUT INS
syntax keyword SqlCmd2 INSERT INSTANCE INSTANTIABLE INSTEAD INTEGERDATE INTERSECT INTO INVOKER
syntax keyword SqlCmd2 IOCOUNT IS ISOLATION ITERATE 
syntax keyword SqlCmd2 JAR JAVA JOIN JOURNAL 
syntax keyword SqlCmd2 KANJI1 KANJISJIS KBYTES KEEP KEY KILOBYTES 
syntax keyword SqlCmd2 LANGUAGE LAST LATIN LEAVE LEFT LENGTH LEVEL LIKE LIMIT LOADING LOCAL
syntax keyword SqlCmd2 LOCATOR LOCK LOCKEDUSEREXPIRE LOCKING LOGGING LOGON LOOP LOW 
syntax keyword SqlCmd2 MACRO MAP MATCHED MAXCHAR MAXIMUM MAXLOGONATTEMPTS MEDIUM MEMBER MERGE
...
syntax keyword SqlCmd2 ORDER ORDERING OUT OUTER OVERRIDE 
...
syntax keyword SqlCmd2 REVALIDATE REVOKE RIGHT RIGHTS ROLE ROLLBACK ROLLFORWARD ROLLUP ROW ROWID
syntax keyword SqlCmd2 RULES RULESET 
syntax keyword SqlCmd2 SAMPLE SAMPLEID SCROLL SECURITY SEL SELECT SELF SEQUENCED SERIALIZABLE SESSION
...
syntax keyword SqlCmd2 UNDEFINED UNDO UNICODE UNION UNIQUE UNKNOWN UNTIL UPD UPDATE USE USER USING 
syntax keyword SqlCmd2 VALIDTIME VALUE VALUES VARIANT_TYPE VIEW VOLATILE 
syntax keyword SqlCmd2 WARNING WHEN WHERE WHILE WORK WORKLOAD WRITE 
syntax keyword SqlCmd2 XML XMLPLAN 
syntax keyword SqlCmd2 ZONE 


syntax keyword SqlOperator EQ GE GT LE LT NE


syntax match BteqCmd "^\s*\.ABORT\>"
syntax match BteqCmd "^\s*\.EXIT\>"
syntax match BteqCmd "^\s*\.EXPORT\>"
syntax match BteqCmd "^\s*\.GOTO\>"
...
syntax keyword BteqKW FILE CONTROLS CONTROL ERRORMAP VERSIONS VERSION ACTIVITYCO ACTIVITYCOUNT 
...
highlight link SqlFunc Statement
